...
const seoCopy = computed(() => {
  ...
  if (locale.value === 'en') {
    return {
      title: 'Beauty Experts in Yerevan | Meet Our Masters ',
...
const mastersPageKey = computed(() => `${locale.value}:${brand.value}`)
...
onMounted(async () => {
  if (mastersStore.initializedKey === mastersPageKey.value && masters.value.length > 0) {
    return
  }
