...
useSeoMeta({
  title: 'Contacts',
  description: 'Адреса, телефоны и график филиалов салона.',
})
  ...
  <section class="section-gap">
    <div class="container-shell space-y-8">
      <div class="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p class="text-xs uppercase tracking-[0.2em] text-sand-600">Reach us</p>
          <h1 class="text-5xl">Контакты</h1>
        </div>
        <NuxtLink to="/booking"><BaseButton>Записаться</BaseButton></NuxtLink>
      </div>
      ...
      <div v-else class="grid gap-4 md:grid-cols-2">
        <Card v-for="branch in branches" :key="branch.id" class="fade-in">
          ...
          <p class="mt-3 text-sm text-[var(--muted)]">{{ branch.address || 'Address TBD' }}</p>
          <p class="mt-1 text-sm text-[var(--muted)]">{{ branch.phone || '+1 000 000 0000' }}</p>
          <p class="mt-4 text-xs uppercase tracking-[0.15em] text-sand-600">График</p>
          ...
          <NuxtLink to="/booking" class="mt-5 inline-block"><BaseButton size="sm">Book now</BaseButton></NuxtLink>
        </Card>
      </div>
